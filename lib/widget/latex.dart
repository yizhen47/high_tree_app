...
class LaTeX extends StatefulWidget {
  ...
  @override
  State<LaTeX> createState() => LaTeXState();
}

class LaTeXState extends State<LaTeX> {
  @override
    ...
    for (int i = 0; i < texts.length; i++) {
      ...
      for (int j = 0; j < subTexts.length; j++) {
        ...
        try {
          ...
          print("HighTree-Debug: LaTeX rendering failed for: ${subTexts[j].trim()}");
          print("HighTree-Debug: Error: $e");
          
          widgetSpans.add(
            TextSpan(
              text: subTexts[j].trim(),
              style: widget.equationStyle ?? widget.laTeXCode.style,
            ),
          );
        }
      }
